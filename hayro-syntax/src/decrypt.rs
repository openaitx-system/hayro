...
impl CryptFilter {
    ...
    fn from_dict(dict: &Dict) -> Option<Self> {
        let method = dict.get::<Name>(CFM)
            .and_then(|name| CryptMethod::from_name(&name))
            .unwrap_or(CryptMethod::None);
        
        let auth_event = dict.get::<Name>(AUTH_EVENT)
            .and_then(|name| AuthEvent::from_name(&name))
            .unwrap_or(AuthEvent::DocOpen);
...
impl fmt::Display for DecryptError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DecryptError::MissingEntry { typ, field } => write!(f, "Missing entry '{}' in {}", field, typ),
        }
    }
}
...
impl Decoder {
    ...
    pub fn from_password(dict: &CryptDict, id: &[u8], pass: &[u8]) -> Result<Decoder> {
        ...
        let (key_bits, method) = match dict.v {
            ...
            4 ..= 6 => {
                let default_filter_name = dict.default_crypt_filter.as_ref()
                    .ok_or_else(|| DecryptError::MissingEntry { 
                        typ: "Encrypt", 
                        field: "StmF".to_string() 
                    })?;

                let default = dict.crypt_filters.get(default_filter_name)
                    .ok_or_else(|| DecryptError::MissingEntry { 
                        typ: "Encrypt", 
                        field: format!("crypt filter entry {:?}", default_filter_name) 
                    })?;
                ...
                match default.method {
                    ...
                    _m => return Err(DecryptError::UnsupportedVersion),
                }
            }
            _v => return Err(DecryptError::UnsupportedVersion),
        };
        let level = dict.r;
        ...
        if level <= 4 {
            ...
            if check_password_rc4(level, &dict.u, id, &key[..std::cmp::min(key_size, 16)]) {
            ...
            let password_unicode = String::from_utf8(pass.to_vec())
                .map_err(|_| DecryptError::InvalidPassword)?;
            let password_prepped = stringprep::saslprep(&password_unicode)
                .map_err(|_| DecryptError::InvalidPassword)?;
            ...
            let intermediate_key_bytes: [u8; 32] = intermediate_key.into();
            let key_slice = Aes256CbcDec::new(GenericArray::from_slice(&intermediate_key_bytes), zero_iv)
        ...
        match self.method {
            ...
            CryptMethod::AESV2 => {
                ...
                let cipher = Aes128CbcDec::new_from_slices(key, iv)
                    .map_err(|_| DecryptError::DecryptionFailure)?;
                Ok(cipher
